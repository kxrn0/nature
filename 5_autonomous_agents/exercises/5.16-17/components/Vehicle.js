...
export default function Vehicle(
  ...
  this.vision = {
    ...
    angle: { span: visionSpan, start: 0, end: 0 },
    clearView: {
      radius: visionClearRadius,
      angle: { span: visionClearAngle, start: 0, end: 0 },
    },
  };
...
Vehicle.prototype.update_vision = function () {
  ...
  const start = angle - span2;
  const end = angle + span2;
  const cSpan2 = this.vision.clearView.angle.span / 2;
  const mid = (start + end) / 2;

  this.vision.angle.start = start;
  this.vision.angle.end = end;

  this.vision.clearView.angle.start = mid - cSpan2;
  this.vision.clearView.angle.end = mid + cSpan2;
};
...
Vehicle.prototype.compute_clearview = function (other) {
  const isInFov = this.is_blocked(other.position);

  if (!isInFov) return;

  const vector = Vector.from_segment(this.position, other.position);
  const inv = new Vector(-this.velocity.y, this.velocity.x);
  const dot = vector.dot(inv);
  const desired = new Vector(0, 0);

  if (dot > 0) desired.set(-vector.y, vector.x);
  else desired.set(vector.y, -vector.x);
...
Vehicle.prototype.is_blocked = function (target) {
  return is_point_in_sector(
    target,
    this.position,
    this.vision.clearView.radius,
    this.vision.clearView.angle.start,
    this.vision.clearView.angle.end
  );
};
